...
/datum/surgery_step/brain/saw_skull
	...
	can_use(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		return ..() && target_zone == "head" && target.op_stage.brain == 1
...
/datum/surgery_step/brain/cut_brain
	...
	can_use(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		return ..() && target.op_stage.brain == 2
...
/datum/surgery_step/brain/saw_spine
	...
	can_use(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		return ..() && target.op_stage.brain == 3
...
/datum/surgery_step/metroid/cut_flesh
	...
	can_use(mob/user, mob/living/carbon/metroid/target, target_zone, obj/item/tool)
		return ..() && target.op_stage.brain == 0
...
/datum/surgery_step/metroid/cut_innards
	...
	can_use(mob/user, mob/living/carbon/metroid/target, target_zone, obj/item/tool)
		return ..() && target.op_stage.brain == 1
...
/datum/surgery_step/metroid/saw_core
	...
	can_use(mob/user, mob/living/carbon/metroid/target, target_zone, obj/item/tool)
		return ..() && target.op_stage.brain == 2 && target.cores > 0
...
/datum/surgery_step/generic/cut_away
	...
	begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		var/datum/organ/external/affected = target.get_organ(target_zone)
		var/msg = "[user] is beginning to saw away [target]'s [affected.display_name] with [tool]."
		var/self_msg = "You begin to saw away [target]'s [affected.display_name] with [tool]!"
		user.visible_message(msg, self_msg)

	end_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		var/datum/organ/external/affected = target.get_organ(target_zone)
		var/msg = "\blue [user] finishes sawing [target]'s [affected.display_name] with [tool]!"
		var/self_msg = "\blue You finish sawing [target]'s [affected.display_name] with [tool]!"
		user.visible_message(msg, self_msg)
	...
	fail_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		var/datum/organ/external/affected = target.get_organ(target_zone)
		user.visible_message("\red [user]'s hand slips, sawing [target]'s [affected.display_name] in a horrible way with \the [tool]!", \
		"\red Your hand slips, sawing [target]'s [affected.display_name] in a horrible way with \the [tool]!")
		affected.createwound(CUT, 20)
...
/datum/surgery_step/limb/clamp_bleeders
	...
	begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		var/datum/organ/external/affected = target.get_organ(target_zone)
		var/msg = "\red [user] is beginning to clamp bleeders in the stump where [target]'s [affected.display_name] used to be with [tool]."
		var/self_msg = "\red [user] begins to clamp bleeders in the stump where [target]'s [affected.display_name] used to be with [tool]!"
		user.visible_message(msg, self_msg)

	end_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		var/datum/organ/external/affected = target.get_organ(target_zone)
		var/msg = "\blue [user] finishes clamping bleeders in the stump where [target]'s [affected.display_name] used to be with [tool]!"
		var/self_msg = "\blue You finish clamping bleeders in the stump where [target]'s [affected.display_name] used to be with [tool]!"
		user.visible_message(msg, self_msg)
		affected.bandage()
		affected.status &= ~ORGAN_BLEEDING
...
/datum/surgery_step/ribcage
	can_use(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		return target_zone == "chest" && hasorgans(target) && target.op_stage.stomach == 0
...
/datum/surgery_step/stomach/cut_open
	...
	can_use(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		return ..() && target.op_stage.stomach == 0

	begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		user.visible_message("[user] starts cutting [target]'s stomach with \the [tool].", \
		"You start cutting [target]'s stomach with \the [tool]." )
...
/datum/surgery_step/stomach/mend_closed
	...
	can_use(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		return ..() && target.op_stage.stomach == 1

	begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
		user.visible_message("[user] starts mending [target]'s stomach with \the [tool].", \
		"You start mending [target]'s stomach with \the [tool]." )
		target.custom_pain("The pain in your stomach is living hell!",1)
