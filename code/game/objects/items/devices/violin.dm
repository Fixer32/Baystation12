...
/obj/item/device/violin/Topic(href, href_list)
	...
	if(href_list["newsong"])
		...
		if(href_list["repeat"]) //Changing this from a toggle to a number of repeats to avoid infinite loops.
			...
			var/newline = strip_input(input("Enter your line: ", "violin") as text|null)
			if(!newline)
				return
			...
			var/content = strip_input(input("Enter your line: ", "violin", song.lines[num]) as text|null)
			if(!content)
				return
			...
			var/t = ""
			do
				t = strip_input(input(usr, "Please paste the entire song, formatted:", text("[]", name), t)  as message)
				if(!in_range(src, usr))
					return
