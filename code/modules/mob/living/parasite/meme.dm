...
mob/living/parasite/meme/verb/Thought()
	...
	var/speaker = sanitize_simple(input("Select the voice in which you would like to make yourself heard.", "Voice") as null|text)
	if(!speaker) return

	var/message = sanitize_simple(input("What would you like to say?", "Message") as null)
	if(!message) return
