...
#define HUMAN_CRIT_MAX_OXYLOSS ( (master_controller.minimum_ticks2) /5) //The amount of damage you'll get when in critical condition. We want this to be a 5 minute deal = 300s. There are 100HP to get through, so (1/3)*last_tick_duration per second. Breaths however only happen every 4 ticks.
		...
		for(var/datum/organ/external/E in organs)
			...
			if(E.status & ORGAN_ROBOT && prob(E.brute_dam + E.burn_dam))
				if(E.name == "l_hand" || E.name == "l_arm")
					if(hand && equipped())
						drop_item()
						emote("custom-drops what they were holding, their [E.display_name?"[E.display_name]":"[E]"] malfunctioning!")
...
//		if(dna && dna.mutantrace == "plant")
	...
	proc/handle_regular_status_updates()
		if(stat == DEAD)	//DEAD. BROWN BREAD. SWIMMING WITH THE SPESS CARP
			...
			handle_organs()
			handle_blood()
			if(health <= config.health_threshold_dead || op_stage.brain == 4.0)
				death()
				blinded = 1
				silent = 0
				return 1
